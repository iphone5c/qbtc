...
var RequestData = function(showLoading) {
    ...
    $(document).ready(
        GLOBAL_VAR.G_HTTP_REQUEST = $.ajax({
            type        : "get",
            url         : GLOBAL_VAR.server+"/json/fullPeriod.do",
            dataType    : 'text',
...
function setHttpRequestParam(mark_from, time_type, limit, since){
    var datas = mark_from.split("_");
    var str = "marketCode="+datas[0]+"&coinCode=" + datas[1] + "&step=" + time_type+"&rand=" + Math.round(Math.random() * 100);;
    return str;
}
...
function getRealData() {
    ...
    var url = GLOBAL_VAR.server+"/json/fullDepth.do?" + GLOBAL_VAR.requestParam;
