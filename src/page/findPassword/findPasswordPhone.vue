	...
	<div>
		...
		<div class="box" v-if="step == 'A'">
			...
			<div class="find-wrapper">
				<p class="title">{{$t("findpaw_page.2")}}</p>
				<el-input :placeholder="($t('findpaw_page.37'))" class="account" v-model="accountPhone">
					<el-select v-model="regionSelect" slot="prepend" :placeholder="($t('findpaw_page.27'))" style="width: 110px">
				    	<el-option v-for="(data,index) in phonhRegion" :key="data.region" :value="'+' + (data.region)">+{{data.region}} {{data.number}}</el-option>
				    </el-select>
				</el-input>
				<el-input :placeholder="($t('findpaw_page.38'))" class="verificationCode" :maxlength="6" v-model="phoneCode">
					<el-button slot="append" class="sendCode" v-if="!computedPhoneTime" @click="sendPhoneCode">{{$t("findpaw_page.9")}}</el-button>
					<el-button slot="append" class="sendCode" v-if="computedPhoneTime" :disabled="true">{{computedPhoneTime}}{{$t("findpaw_page.10")}}</el-button>
				</el-input>
				<!-- <el-select v-model="certificatesIndex" placeholder="证件类型" class="certificatesType" style="width:100%;">
				    <el-option v-for="item in certificatesVlaue" :key="item.value" :label="item.name" :value="item.value">
				    </el-option>
				</el-select>
				<el-input placeholder="证件号码（选填）" class="certificatesNum" v-model="certificatesNum"></el-input> -->
				<el-button class="confirm" type="primary" @click="firstStep">{{$t("findpaw_page.11")}}</el-button>
				<div class="register">
					<router-link to="/findPasswordEmail"><p class="findPasswordEmail">{{$t("findpaw_page.3")}}</p></router-link>
...
import {getSMSCode,verifyPhone,isVerificationCode,findPasswordPhone} from '@/utils/api'
...
export default {
	...
	async mounted() {
		...
		// console.log(this.certificatesVlaue);
	},
	computed:{
		 // 判断手机号格式是否正确
	    rightPhone : function(){
	...
	methods:{
		...
		async sendPhoneCode(){
			if(!this.regionSelect){
				this.$message.error(this.$t('findpaw_page.28'));
          		return
			}
			if(!this.accountPhone){
				this.$message.error(this.$t('findpaw_page.29'));
				return
			}
			if(this.regionSelect == "+86"){
				if(!this.rightPhone){
					this.$message.error(this.$t('findpaw_page.30'));
					return
				}
			}
			// 验证手机号是否注册
			...
			if (!exsis.result) { 
		       	this.$message.error(this.$t('findpaw_page.31'));
		       	return
	      	}
		...
		async firstStep(){
			...
			if(!this.accountPhone){
				this.$message.error(this.$t('findpaw_page.32'));
          		return
			}
			if(this.regionSelect == "+86"){
				if(!this.rightPhone){
					this.$message.error(this.$t('findpaw_page.33'));
          		return
				}
			}
			if(!this.phoneCode){
				this.$message.error(this.$t('findpaw_page.34'));
          		return
			}
			// 验证短信验证码是否正确
			let res = await isVerificationCode('1',this.phoneCode);
			if(!res.result){
				this.$message.error(this.$t('findpaw_page.35'));
				return
			}
			this.step = 'B';
		},
		...
		async secondSteps(){
			...
			let res = await findPasswordPhone(this.accountPhone,this.phoneCode,this.password,this.againPassword);
			if(!res){
				return
			}
			this.step = 'C'
